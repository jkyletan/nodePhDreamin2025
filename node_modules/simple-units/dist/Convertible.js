...
export default class Convertible {
    ...
    to(unit) {
        ...
        if (typeof unit === "string") {
            ...
            if (!target)
                throw new ConversionError(`Unit '${unit}' does not belong to group '${this.unit.group.name}'!`);
        }
        else {
            target = unit;
    ...
    format(format, formatOptions) {
        ...
        if (formatOptions === null || formatOptions === void 0 ? void 0 : formatOptions.length)
        ...
        if (length === "short") {
            ...
            if (count === "sg")
                unit = this.unit.format.long.sg[0];
            else
                unit = this.unit.format.long.pl[0];
        }
    ...
    compare(other) {
        if (other.unit.group !== this.unit.group)
            throw new IllegalArgumentsError(`Cannot compare convertibles. Unit '${other}' does not belong to the same group as '${this.unit}'!`);
        const copyOfOther = other.copy()._internal._asUnitUnchecked(this.unit);
        if (this.value < copyOfOther.value)
            return -1;
        else if (this.value === copyOfOther.value)
            return 0;
        else
            return 1;
    }
