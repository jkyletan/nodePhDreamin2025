...
export default class FlexibleUnit {
     ...
     */
    constructor(format, ratio, shift, system, variables) {
        this.format = format;
        this.ratio = ratio;
        this.shift = shift;
        this.system = system;
    ...
    generateCombinations() {
        ...
        return [...product(...valueArrays)];
    }
    /**
     * @hidden
     */
    generateUnitFormat(combo) {
        const newFormat = {
            short: [],
            long: { sg: [], pl: [] },
        };
     ...
     * @hidden
     */
    fillFormat(format, combo, long) {
        let result = "";
        for (let i = 0; i < format.length; i++) {
            if (format[i] === "%" && i + 1 < format.length) {
                const variableIndex = this.cutInteger(format.substr(i + 1));
                if (variableIndex === false)
                    throw new InvalidVariableSyntaxError(`Inside the flexible unit's format a '%' must be followed by a number indicating the variable's index!`);
                const variableIndexNumber = Number(variableIndex);
                if (variableIndexNumber >= combo.length)
                    throw new InvalidVariableSyntaxError(`The specified variable index '%${variableIndexNumber}' is out of range!`);
                if (long)
                    result += combo[variableIndexNumber].long;
                else
                    result += combo[variableIndexNumber].short;
...
//# sourceMappingURL=FlexibleUnit.js.map